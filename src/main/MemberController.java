/**
 * This class stores and manages the MemberData objects which correspond to the members of the
 * ChocAn network.
 *
...
public class MemberController {
  ...
  /**
   * ArrayList which stores the MemberData objects that correspond to the members of the ChocAn
   * network.
   */
  private static ArrayList<MemberData> memberList = new ArrayList<MemberData>();

  /**
   * Prompts the user for information on a member, which then is funneled into a MemberData
   * constructor. The new MemberData object is then stored into the arraylist.
   */
  ...
  /**
   * Prompts the user for a member number, and then deletes the MemberData object from the arraylist
  ...
  /**
   * Prompts the user to input a member number, after which the user is prompted to edit a certain
   * field of the MemberData which corresponds to the given member number.
   */
  public static void updateMember() {
    ...
    while (true) {
      ...
      if (memberNumber.equals("-1")) return;
        ...
        if (memberList.get(i).getMemberNumber().equals(memberNumber)) {
          while (fieldChoice != -1) {
            ...
            switch (fieldChoice) {
              ...
              case 5:
                IOController.output(
                    "Member suspension will be toggled from "
  ...
  /**
   * Prompts a the user for a member number, then checks to see if that member number corresponds to
   * any MemberData which exists in the array list.
   *
   ...
   * parameter and returns whether or not it's a valid member number without prompting the user or
  ...
  public static boolean validateCode(String memberCode) {
    for (int i = 0; i < memberList.size(); i++) {
      if (memberList.get(i).getMemberNumber().equals(memberCode)) return true;
    }
   ...
   *
   * @param memberNumber given member number used to isolate the MemberData object that will have a
   *     service added to it
  ...
  /**
   * Get function for the ArrayList<MemberData> which corresponds to the members who are a part of
   * the ChocAn network.
   *
