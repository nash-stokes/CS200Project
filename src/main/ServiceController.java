/**
 * This class is responsible for holding the services that have been performed, modifying the
 * storage of the services, and handling the provider directory.
 *
...
public class ServiceController {
  ...
  /**
   * Prompts the user to add a service to the service data arraylist. The method continually prompts
   * for information then aggregates it into a ServiceData constructor, at which point it saves that
   * new ServiceData object into the arraylist. Furthermore, it stores the ServiceData into the
   * proper provider/member service arraylists.
   */
  public static void addService(String providerNumber) {
    ...
    while (!memberValidated && !memberNumber.equals("-1")) {
      ...
      if (MemberController.validateCode(memberNumber)) memberValidated = true;
      else IOController.output("Invalid member number. Retry, or enter -1 to exit.");
    }
    ...
    if (serviceCode.equals("-1")) return;
    ...
    ServiceData x =
        new ServiceData(
            ...
            serviceCode,
            comment,
            ProviderDirectoryData.fillFee(serviceCode),
            MemberController.fillName(memberNumber),
            ProviderDirectoryData.fillName(serviceCode),
            ProviderController.fillName(providerNumber));
  ...
  /**
   * Attempts to write the provider directory data into a file called Provider_Directory.txt. If
   * this is unable to happen, it excepts and lets the user know that the file was unable to be
   * found.
   */
