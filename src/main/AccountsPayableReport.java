  ...
  */
  public static void printAccountsPayableReport() throws IOException {
    ...
    for (int x = 0; x < ProviderController.getProviderList().size(); x++) {
        ...
        feeTotal = 0;
        for (int i = 0; i < ProviderController.getProviderList().get(x).getServiceList().size(); i++) {
          feeTotal += ProviderController.getProviderList().get(x).getServiceList().get(i).getServiceFee();
        }
