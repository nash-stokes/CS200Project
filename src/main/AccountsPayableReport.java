...
/**
 * Accounts Payable Report is a report for managers to see which accounts are payable. Once called,
 * it will create a text file and save it onto the terminal for it to be sent as email.
 *
...
public class AccountsPayableReport {
  ...
  /**
   * The variable finalConsults is the overall number of services that all the providers has
   ...
   * objects and write the information in the text file. It will create a file, open it, and write
   * in the necessary information in the correct format .
   *
  ...
  public static void printAccountsPayableReport() throws IOException {
    ...
    for (int x = 0; x < ProviderController.getProviderList().size(); x++) {
      numOfConsults = ProviderController.getProviderList().get(x).getServiceList().size();
      feeTotal = 0;
      for (int i = 0;
          i < ProviderController.getProviderList().get(x).getServiceList().size();
          i++) {
        feeTotal +=
            ProviderController.getProviderList().get(x).getServiceList().get(i).getServiceFee();
      }
      writing.println(
          ProviderController.getProviderList().get(x).getName()
              + " | "
              + numOfConsults
              + " | "
              + feeTotal);
